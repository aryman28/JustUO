...
namespace Server.Items
{
	...
	public class PublicStablePost : Item
	{
		[CommandProperty( AccessLevel.GameMaster )]
		public int MinRange { get { return 1; } } // change this to adjust the min wander area (affects gump settings too)
		
		[CommandProperty( AccessLevel.GameMaster )]
		public int MaxRange { get { return 4; } } // change this to adjust the max wander area (affects gump settings too)
		
		[CommandProperty( AccessLevel.GameMaster )]
		public static int DefaultWander { get { return 1; } } //change this to adjust the default wander area (only affects initial setup)
		
		[CommandProperty( AccessLevel.GameMaster )]
		public Mobile Owner{ get{ return m_Owner; } set{ m_Owner = value; } }

                [CommandProperty( AccessLevel.GameMaster )]
		public BaseMount Controlled{ get{ return m_Controlled; } set{ m_Controlled = value; } }
		
		private BaseMount m_Controlled;
		...
		#region Constructors
		[Constructable]
		public PublicStablePost() : base( 0x14E7 )
		{
			Name = "Publiczny slupek do przywiazywania";
			Movable = false;
		}

		[Constructable]
		public PublicStablePost( bool east ) : base( east ? 0x14E7 : 0x14E8 )
		{
			Name = "Publiczny slupek do przywiazywania";
			Movable = false;
		}

		public PublicStablePost( Serial serial ) : base( serial )
		{
		}
		#endregion
...
if( from.InRange( this.GetWorldLocation(), 1 ) == false )
{
...
else
{
			if ( Owner == null )
			{
				from.Target = new PublicStablePostTarget( this );
		...
		public override void Delete()
		{
			if ( Controlled == null )
				...
				Say("You must release the creature controlled before you can remove me.");
		}
		...
		public override void OnMovement( Mobile m, Point3D oldLocation )
		{
			...
			if ( m is BaseMount )
			{
				if ( (BaseMount)m == Controlled )
				{
					if ( ( (BaseMount)m ).Loyalty <= 100 )
						( (BaseMount)m ).Loyalty = 100;
					if ( ( (BaseMount)m ).RangeHome != HomeRange )
						( (BaseMount)m ).RangeHome = HomeRange;
				}
			}
		}
        ...
        public override void GetContextMenuEntries(Mobile from, List<ContextMenuEntry> list)
		{
			...
			else
			{
				...
				else
				{
					...
					PublicStablePost.GetContextMenuEntries( from, this, list );
				}
			}
		}
		...
		private class MenuEntry : ContextMenuEntry
		{
			...
			{
				// send gump
				m_Mobile.CloseGump( typeof( StableGump ) );
				m_Mobile.SendGump( new StableGump( m_Item ) );
			}
		}
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				...
				case 0:
				{
					...
					m_Controlled = (BaseMount)reader.ReadMobile();
		...
		private class PublicStablePostTarget : Target
		{
			private PublicStablePost m_Post;

			public PublicStablePostTarget( PublicStablePost p ) : base( 10, false, TargetFlags.None )
			{
				m_Post = p;
			}
			...
			{
				if ( target == from )
					m_Post.Say( "You can't hitch yourself!" );
				else if ( target is PublicStablePost )
				{
					if ( (PublicStablePost)target == m_Post )
					{
						...
						else
						{
							...
							if ( house != null && ( house.IsOwner( from ) || from.AccessLevel >= AccessLevel.GameMaster ) )
							{
								...
								from.SendMessage( "You redeed your PublicStablePost." );

								from.AddToBackpack( new PublicStablePostDeed( m_Post.East ) );
								m_Post.Delete();
							}
						}
					}
					...
					m_Post.Say( "You can't hitch other players to the post!" );
				else if ( target is Item )
					m_Post.Say( "This post is not for items." );
				else if ( Server.Spells.SpellHelper.CheckCombat( from ) )
					m_Post.Say( "You cannot do that while you're fighting." );
				else if ( ( target is BaseMount ) && m_Post.Controlled == null )
				{
					BaseMount c = (BaseMount)target;
				...
				{
					BaseMount c = null;
					if ( m_Post.Controlled == (BaseMount)target && m_Post.Owner == from )
					{
						c = (BaseMount)target;
						...
						from.SendMessage( "This post already has a creature PublicStable. (Unstable the other first)" );
				}
				else
					from.SendMessage( "This post is for mounts only." );
			}
		}
	}

	[Flipable( 0x14F0, 0x14EF )]
	public class PublicStablePostDeed : Item
	{
		private bool m_East;
		public bool East { get { return m_East; } set { m_East = value; } }

		[Constructable]
		public PublicStablePostDeed() : base( 0x14F0 )
		{
			East = Utility.RandomBool();
			if ( East )
				Name = "Public hitching post east deed";
			else
				Name = "Public hitching post south deed";
			Weight = 1.0;
		}

		[Constructable]
		public PublicStablePostDeed( bool east ) : base( 0x14F0 )
		{
			East = east;
			if ( East )
				Name = "Public hitching post east deed";
		...
		public PublicStablePostDeed( Serial serial ) : base( serial )
		{
		}
		...
		{
			private PublicStablePostDeed m_Deed;

			public InternalTarget( PublicStablePostDeed deed ) : base( -1, true, TargetFlags.None )
			{
				m_Deed = deed;
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				PublicStablePost post = new PublicStablePost( m_Deed.East );
...
namespace Server.Gumps
{
	public class StableGump : Gump
	{
		private PublicStablePost m_Post;
		private int temp;
		public StableGump( PublicStablePost post ) : base( 0, 0 )
		{
			m_Post = post;
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			switch ( button )
			{
				case 1:
				{
					...
					sender.Mobile.CloseGump( typeof( StableGump ) );
					sender.Mobile.SendGump( new StableGump( m_Post ) );
					break;
				}
				case 2:
				{
