...
namespace Server.Scripts.Commands
{
  public class Hungry
  {
     ...
     {
        CommandSystem.Register( "Glod", AccessLevel.Player, new CommandEventHandler( Hungry_OnCommand ) );
        CommandSystem.Register( "Pragnienie", AccessLevel.Player, new CommandEventHandler( Hungry_OnCommand ) );
     }

     [Usage( "Glod|| Pragnienie" )]
...
public class gumpfaim : Gump
{
 ...
 public gumpfaim(Mobile from) : base(0,0)
 {
  ...
  AddLabel( 60, 62, from.Hunger < 6 ? 33 : 0, string.Format( "Glod: {0} / 20", from.Hunger));
  AddLabel( 60, 81, from.Thirst < 6 ? 33 : 0, string.Format( "Pragnienie: {0} / 20", from.Thirst));
  AddItem( 8, 78, 8093);
  AddItem( 19, 60, 4155);
 }
