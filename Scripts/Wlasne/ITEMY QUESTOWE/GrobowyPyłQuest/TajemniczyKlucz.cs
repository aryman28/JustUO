...
{
    public enum TajemniczyKluczType
    {
        Copper = 0x100E,
        Gold = 0x100F,
        Iron = 0x1010,
    ...
    public interface IYLockable
    {
        bool Locked { get; set; }
        uint KeyValue { get; set; }
    }

    public class TajemniczyKlucz : Item
    {
        ...
        [Constructable]
        public TajemniczyKlucz()
            : this(TajemniczyKluczType.Gold, 0)
        {
        }

        [Constructable]
        public TajemniczyKlucz(TajemniczyKluczType type)
            : this(type, 0)
        {
        }

        [Constructable]
        public TajemniczyKlucz(uint val)
            : this(TajemniczyKluczType.Gold, val)
        {
        }

        [Constructable]
        public TajemniczyKlucz(TajemniczyKluczType type, uint LockVal)
        ...
        public TajemniczyKlucz(TajemniczyKluczType type, uint LockVal, Item link)
            : base((int)type)
        {
            this.Weight = 1.0;
            this.Name = "Tajemniczy Klucz";
            this.Hue = 33;
            this.m_MaxRange = 3;
            //this.m_KeyVal = LockVal;
            this.m_Link = link;
        ...
        public TajemniczyKlucz(Serial serial)
            : base(serial)
        {
        }
        ...
        public static void RemoveKeys(Container cont, uint keyValue)
        {
            ...
            {
                if (item is TajemniczyKlucz)
                {
                    TajemniczyKlucz key = (TajemniczyKlucz)item;
        ...
        public bool UseOn(Mobile from, IYLockable o)
        {
            if (o.KeyValue == this.KeyValue)
            {
                    if (o is TajemniczyKufer)
                    {
        ...
        {
            private readonly TajemniczyKlucz m_Key;
            public RenamePrompt(TajemniczyKlucz key)
            {
                this.m_Key = key;
            }
        ...
        {
            private readonly TajemniczyKlucz m_Key;
            public UnlockTarget(TajemniczyKlucz key)
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                else if (targeted is IYLockable)
                {
                    if (this.m_Key.UseOn(from, (IYLockable)targeted))
                        number = -1;
                    else
        ...
        {
            private readonly TajemniczyKlucz m_Key;
            public CopyTarget(TajemniczyKlucz key)
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (targeted is TajemniczyKlucz)
                {
                    TajemniczyKlucz k = (TajemniczyKlucz)targeted;
