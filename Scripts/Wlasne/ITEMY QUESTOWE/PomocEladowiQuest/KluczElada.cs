...
namespace Server.Items
{
    ...
    public class KluczElada : Item
    {
        ...
        [Constructable]
        public KluczElada()
            : this(KluczType.Iron, 0)
        {
        }

        [Constructable]
        public KluczElada(KluczType type)
            : this(type, 0)
        {
        }

        [Constructable]
        public KluczElada(uint val)
            : this(KluczType.Iron, val)
        {
        }

        [Constructable]
        public KluczElada(KluczType type, uint LockVal)
        ...
        public KluczElada(KluczType type, uint LockVal, Item link)
            : base((int)type)
        {
            this.Weight = 1.0;
            this.Name = "Klucz Elada";
            this.Hue = 33;
            this.m_MaxRange = 3;
            //this.m_KeyVal = LockVal;
            this.m_Link = link;
        ...
        public KluczElada(Serial serial)
            : base(serial)
        {
        }
        ...
        public static void RemoveKeys(Container cont, uint keyValue)
        {
            ...
            {
                if (item is KluczElada)
                {
                    KluczElada key = (KluczElada)item;
        ...
        public bool UseOn(Mobile from, IXLockable o)
        {
        ...
        {
            private readonly KluczElada m_Key;
            public RenamePrompt(KluczElada key)
            {
                this.m_Key = key;
            }
        ...
        {
            private readonly KluczElada m_Key;
            public UnlockTarget(KluczElada key)
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                else if (targeted is IXLockable)
                {
                    if (this.m_Key.UseOn(from, (IXLockable)targeted))
                        number = -1;
                    else
        ...
        {
            private readonly KluczElada m_Key;
            public CopyTarget(KluczElada key)
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (targeted is KluczElada)
                {
                    KluczElada k = (KluczElada)targeted;
