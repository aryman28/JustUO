	...
	public class ShrinkItem : Item, IShrinkItem
	{
		...
		public bool IsStatuette
		{
			...
			set
			{
				...
				else if ( m_IsStatuette = value )
				{
					...
					Name = "zmniejszone stworzenie";
				}
				else
				{
					ItemID = 0x14EF;
					Name = "akt wlasnosci stworzenia";
				}
			}
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( m_Locked && m_Owner != from )
			{
				...
				from.SendMessage( "Posiadasz zbyt wiele stworzen aby to powiekszyc." );

			else if ( Server.Spells.SpellHelper.CheckCombat( from ) )
				from.SendMessage( "Nie mozesz powiekszyc stworzenia w czasie walki." );
				...
				from.SendMessage( "Nie masz wymaganego skilla by kontrolowac to stworzenie.");
		...
		public override void AddNameProperties( ObjectPropertyList list )
		{
			...
			{
				list.Add( 1060663, "NAZWA\t{0} RASA: {1} PLEC: {2}", m_Name, m_Breed, m_Gender );
				//list.Add( 1060663, "NAZWA\t{0} PLEC: {1}", m_Name, m_Gender );
				list.Add( 1061640, ( null == m_Owner ) ? "nikt (DZIKIE)" : m_Owner.Name ); // Owner: ~1_OWNER~
				list.Add( 1060659, "STATYSTYKA\tStr {0}, Dex {1}, Int {2}", m_RawStr, m_RawDex, m_RawInt );
				list.Add( 1060660, "BOJOWE\tWrestling {0}, Taktyka {1}, Anatomia {2}, Zatruwanie {3}", m_Wrestling, m_Tactics, m_Anatomy, m_Poisoning );
				list.Add( 1060661, "MAGICZNE\tMagia {0}, MiaraIntelektu {1}, OdpornoscNaMagie {2}, Medytacja {3}", m_Magery, m_EvalInt, m_MagicResist, m_Meditation );
				if ( !( 0 == m_Parry && 0 == m_Archery ))
					list.Add( 1060661, "WALKI\tLucznictwo {0}, Szermierka {1}, Obuchy {2}, Parowanie {3}, Miecze {4}", m_Archery, m_Fencing, m_Macing, m_Parry, m_Swords );
				if ( m_EvoEp > 0 )
					list.Add( 1060662, "EP\t{0}, Stage: {1}", m_EvoEp, m_EvoStage + 1 );
			}
			else
				list.Add( 1060663, "NAZWA\t{0} RASA: {1}", m_Name, m_Breed );
		}
		...
		private void PreloadProperties()
		{
			...
			m_Gender = (m_Pet.Female ? "Zenska" : "Meska");
			...
			m_Wrestling = m_Pet.Skills[SkillName.Boks].Base;
			m_Tactics = m_Pet.Skills[SkillName.Taktyka].Base;
			m_Anatomy = m_Pet.Skills[SkillName.Anatomia].Base;
			m_Poisoning = m_Pet.Skills[SkillName.Zatruwanie].Base;
			m_Magery = m_Pet.Skills[SkillName.Magia].Base;
			m_EvalInt = m_Pet.Skills[SkillName.Intelekt].Base;
			m_MagicResist = m_Pet.Skills[SkillName.ObronaPrzedMagia].Base;
			m_Meditation = m_Pet.Skills[SkillName.Medytacja].Base;
			m_Parry = m_Pet.Skills[SkillName.Parowanie].Base;
			m_Archery = m_Pet.Skills[SkillName.Lucznictwo].Base;
			m_Fencing = m_Pet.Skills[SkillName.WalkaSzpadami].Base;
			m_Swords = m_Pet.Skills[SkillName.WalkaMieczami].Base;
			m_Macing = m_Pet.Skills[SkillName.WalkaObuchami].Base;
