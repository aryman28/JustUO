...
{
    public class HurricaneSpell : DruidSpell
    {
        ...
        public override string SpellDescription
        {
            get
            {
                return "Tworzysz tornado które zadaje obrażenia wszystkim celom w jego pobliżu.";
            }
        }
        public HurricaneSpell(Mobile caster, Item scroll)
        ...
        public void Target(IPoint3D p)
        {
            ...
            else if (SpellHelper.CheckTown(p, Caster) && CheckSequence())
            {
                ...
                Effects.PlaySound(p, Caster.Map, 0x15);
                ...
                if (Core.AOS)
                    duration = TimeSpan.FromSeconds((12 + (Caster.Skills.WiedzaOBestiach.Fixed / 5)) / 4);
                else
                    duration = TimeSpan.FromSeconds(3.0 + (Caster.Skills[SkillName.WiedzaOBestiach].Value * 0.5));
        ...
        private class InternalItem : Item
        {
            ...
            public override bool OnMoveOver(Mobile m)
            {
                if (Visible && m_Caster != null && (!Core.AOS || m != m_Caster) && SpellHelper.ValidIndirectTarget(m_Caster, m) && m_Caster.CanBeHarmful(m, false))
                {
                    ...
                    if (!Core.AOS && m.CheckSkill(SkillName.ObronaPrzedMagia, 0.0, 100.0))
                    {
                        damage = 1;
                    ...
                    AOS.Damage(m, m_Caster, damage, 0, 0, 50, 0, 50);
                    m.PlaySound(0x15);
                }
                    ...
                    else
                    {
                        ...
                        if (map != null && caster != null)
                        {
                            ...
                            while (m_Queue.Count > 0)
                            {
                                ...
                                AOS.Damage(m, caster, damage, 0, 0, 50, 0, 50);
                                m.PlaySound(0x15);
                            }
                        }
                    }
        ...
        {
            private HurricaneSpell m_Owner;

            public InternalTarget(HurricaneSpell owner)
