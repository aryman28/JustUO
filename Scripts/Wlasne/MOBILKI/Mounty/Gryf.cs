...
{
	[CorpseName( "zwloki gryfa" )]
	public class Gryf : BaseMount
	{
		...
		[Constructable]
		public Gryf()
			: base( "gryf", 0xFE, 0x3F0B, AIType.AI_Melee, FightMode.Closest, 10, 1, 0.2, 0.4 )
		{
			Hue = GetHue();
                        Body = 254;
			...
			SetSkill( SkillName.Anatomia, 75.1, 80.0 );
			SetSkill( SkillName.ObronaPrzedMagia, 85.1, 100.0 );
			SetSkill( SkillName.Taktyka, 100.1, 110.0 );
			SetSkill( SkillName.Boks, 100.1, 120.0 );
		...
		{
			if ( ControlMaster.Skills[SkillName.Fanatyzm].Base >= 90.0 )
				return true;
			return false;
		}
		...
		public override double GetControlChance( Mobile m, bool useBaseSkill )
		{
			...
			double skill = (useBaseSkill? m.Skills.Fanatyzm.Base : m.Skills.Fanatyzm.Value);
