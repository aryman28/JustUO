...
namespace Server.Engines.XmlSpawner2
{
    public class XmlCustomAttacks : XmlAttachment
    {
        ...
        public static new void Initialize()
        {
            ...
            AddSpecialAttack("Krwawi¹ce rozciêcie", "Cios k³uty/ciêty powoduj¹cy krwawienie",        // attack name, and description
                SpecialAttacks.KrwawiaceRozciecie, 0x5201, TimeSpan.FromSeconds(3.5),                   // attack id, id of gump icon, and chaining time
                0, 30, 0, 0,                                                                    // mana, stam, hits, karma usage
                40,30,0,                                                                        // str, dex, int requirements
                new SkillName [] { SkillName.Taktyka },                                        //  skill requirement list
                new int [] { 90 },                                                               // minimum skill levels
                null,
                null                                                                // reagent quantities
            );

            AddSpecialAttack("Potrójne uderzenie", "Zadajesz potrójne obra¿enia",        // attack name, and description
            ...
            AddSpecialAttack("Przebicie pancerza", "Omija pancerz przeciwnika",        // attack name, and description
                SpecialAttacks.PrzebiciePancerza, 0x5200, TimeSpan.FromSeconds(3.5),                   // attack id, id of gump icon, and chaining time
                0, 20, 0, 0,                                                                    // mana, stam, hits, karma usage
                30,30,0,                                                                        // str, dex, int requirements
                new SkillName [] { SkillName.Taktyka },                                        //  skill requirement list
            ...
            AddSpecialAttack("Splecione ostrza", "Wykonujesz szybkie ciêcia zadaj¹c dodatkowe obra¿enia",        // attack name, and description
                SpecialAttacks.SzybkieCiecie, 0x5217, TimeSpan.FromSeconds(2),                   // attack id, id of gump icon, and chaining time
                0, 15, 0, 0,                                                                    // mana, stam, hits, karma usage
                10,30,0,                                                                        // str, dex, int requirements
                new SkillName [] { SkillName.Taktyka },                                        //  skill requirement list
                new int [] { 50 },                                                               // minimum skill levels
                null,
                null                                                                // reagent quantities
            ...
            AddSpecialAttack("Przebicie Zbroi", "Przebija zbrojê przeciwnika",        // attack name, and description
                SpecialAttacks.PrzebicieZbroi, 0x5216, TimeSpan.FromSeconds(3.5),                   // attack id, id of gump icon, and chaining time
                0, 20, 0, 0,                                                                    // mana, stam, hits, karma usage
                40,30,0,                                                                        // str, dex, int requirements
                new SkillName [] { SkillName.Taktyka },                                        //  skill requirement list
            ...
            AddSpecialAttack("Dren umys³u", "Wysysasz manê przeciwnika",
                ...
                10, 5, 5, 0,
                0,0,40,
                new SkillName [] { SkillName.Magia },
                new int [] { 50 },
                null,
                null
            );
            AddSpecialAttack("Atak ¿ywio³u", "Przywo³uje ¿ywio³aka energii aby wspomóg³ ciê w walce",
            ...
            AddSpecialAttack("Drain staminy", "Wysysasz staminê przeciwnikowi",
            ...
            AddSpecialAttack("Parali¿uj¹cy Strach", "Strach parali¿uje przeciwnika i zmusza go do ucieczki",
                SpecialAttacks.ParalyzingFear, 0x500d, TimeSpan.FromSeconds(3.5),
                10, 10, 5, 10,
                0, 0, 40,
                new SkillName [] { SkillName.Nekromancja },
                new int [] { 30 },
                new Type [] {typeof(Head)},
                new int [] { 1 }
            );

            AddSpecialAttack("Dar ¯ycia", "Ataki odnawiaj¹ twoje ¿ycie",
                SpecialAttacks.GiftOfHealth, 0x500c, TimeSpan.FromSeconds(3.5),
                40, 20, 0, 0,
                0, 0, 30,
                new SkillName [] { SkillName.Leczenie },
                new int [] { 60 },
            ...
            AddSpecialAttack("Puff of Smoke", "Makes the attacker invisible",
                ...
                20, 40, 0, 0,
                0, 40, 0,
                new SkillName [] { SkillName.Zakradanie, SkillName.Ukrywanie },
                new int [] { 50, 50 },
        ...
        public void DoSpecialAttack(Mobile attacker, Mobile defender, BaseWeapon weapon, int damageGiven, SpecialAttack special)
		{
            ...
            attacker.SendMessage("Wykonano {0}!", special.Name);
            defender.SendMessage("Wykonano {0}!", special.Name);
            ...
            switch(special.AttackID)
            {
                ...
                {
                    defender.Damage( weapon.MinDamage*3, attacker );
                    //attacker.SendMessage("WykonaÅ‚eÅ› potrÃ³jne uderzenie"); // You attack with lightning speed!
			        //defender.SendMessage("OtrzymaÅ‚eÅ› potrÃ³jne obra¿enia!"); // Your attacker strikes with lightning speed!
                ...
                case SpecialAttacks.PrzebiciePancerza:
                {
                    defender.Damage( weapon.MinDamage*1, attacker );
                    defender.PlaySound(0x56);
                    defender.FixedParticles(0x3728, 200, 25, 9942, EffectLayer.Waist);
                    break;
                }

                case SpecialAttacks.KrwawiaceRozciecie:
                {
                    defender.Damage( weapon.MinDamage*1, attacker );
                    defender.PlaySound(0x56);
                    defender.FixedParticles(0x3728, 200, 25, 9942, EffectLayer.Waist);
                    //defender.Damage( XmlWeaponAbility, BleedAttack, attacker );
                    break;
                }

                case SpecialAttacks.PrzebicieZbroi:
                {
                    defender.Damage( weapon.MaxDamage*1, attacker );
                    defender.PlaySound(0x56);
                    defender.FixedParticles(0x3728, 1, 26, 0x26D6, 0, 0, EffectLayer.Waist);
                    break;
                }

                case SpecialAttacks.SzybkieCiecie:
                {
                    defender.Damage( weapon.MaxDamage+=5, attacker );
                    defender.Damage( weapon.MinDamage+=5, attacker );
		...
		private class CustomAttacksInfoGump : Gump
		{
    		...
    		public CustomAttacksInfoGump( Mobile from, XmlCustomAttacks a, SpecialAttack s) : base( 0,0)
            {
                ...
                text.AppendFormat("\n\nWymagania Statystyki/Umiejêtnoœci:" );
                if(s.StrReq > 0)
                {
                    text.AppendFormat("\n     {0} Si³a", s.StrReq );
                }
                if(s.DexReq > 0)
                {
                    text.AppendFormat("\n     {0} Zrêcznoœæ", s.DexReq );
                }
                if(s.IntReq > 0)
                {
                    text.AppendFormat("\n     {0} Inteligencja", s.IntReq );
                }
                ...
                if(s.StamReq > 0)
                {
                    text.AppendFormat("\n     {0} Wytrzyma³oœæ", s.StamReq );
                }
                if(s.HitsReq > 0)
                {
                    text.AppendFormat("\n     {0} Punkty ¿ycia", s.HitsReq );
                }
                if(s.KarmaReq > 0)
                {
                    text.AppendFormat("\n     {0} Karma", s.KarmaReq );
		...
		private class CustomAttacksGump : Gump
		{
    		...
    		public CustomAttacksGump( Mobile from, XmlCustomAttacks a) : base( 0,0)
            {
                ...
                AddAlphaRegion(0, 0, 60, 65);
                AddImage( 10, 10, 0x290A );
                AddImage( 25, 10, 0x290C );
                AddLabel( 5, 52, 55, "Ataki" );
                AddLabel( 5, 67, 55, "Specjalne" );
