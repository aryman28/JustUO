...
namespace Server.Engines.XmlSpawner2
{
    public class XmlCustomDefenses : XmlAttachment
    {
        ...
        public static new void Initialize()
        {
            ...
            AddSpecialDefense("Kolczasta tarcza", "Odbijasz obra¿enia przeciwnika",
                SpecialDefenses.SpikeShield, 0x5220, TimeSpan.FromSeconds(10.5), // example of using an itemid for the gump icon
                0, 20, 0, 0,
                30,30,0,
                new SkillName [] { SkillName.Parowanie, SkillName.Taktyka },
                new int [] { 50, 50 },
                null,
                null
            );
            ...
            AddSpecialDefense("Mind Drain", "Drains mana from the target",
                SpecialDefenses.MindDrain, 0x522B, TimeSpan.FromSeconds(10.5),  // explicitly specifying the gump icon as a gumpid
                0, 20, 0, 0,
                30,30,0,
                new SkillName [] { SkillName.Parowanie, SkillName.Taktyka },
                new int [] { 50, 50 },
            ...
            AddSpecialDefense("Stam Drain", "Drains stamina from the target",
                SpecialDefenses.StamDrain, 0x522A, TimeSpan.FromSeconds(10.5),                // if the icon type is not specified, gump icons use gumpids
                0, 20, 0, 0,
                30, 40, 0,
                new SkillName [] { SkillName.Parowanie, SkillName.Taktyka },
                new int [] { 50, 50 },
            ...
            AddSpecialDefense("Og³uszenie tarcz¹", "Og³usza i parali¿uje cel",
                SpecialDefenses.ParalyzingFear, 0x5223, TimeSpan.FromSeconds(10.5),
                0, 20, 0, 0,
                50, 50, 0,
                new SkillName [] { SkillName.Parowanie, SkillName.Taktyka },
                new int [] { 50, 50 },
            ...
            AddSpecialDefense("Gift of Health", "Absorbs damage as health",
                SpecialDefenses.GiftOfHealth, 0x5229, TimeSpan.FromSeconds(10.5),
                0, 30, 0, 0,
                30, 30, 0,
                new SkillName [] { SkillName.Parowanie, SkillName.Taktyka },
                new int [] { 50, 50 },
            ...
            AddSpecialDefense("Œciana kurzu", "Tworzy zas³one kurzu i stajesz siê niewidzialny",
        ...
        public int DoSpecialDefense(Mobile attacker, Mobile defender, BaseWeapon weapon, int damageGiven, SpecialDefense special)
		{
            ...
            defender.SendMessage("U¿y³eœ obrony o nazwie - {0}!", special.Name);
            ...
            switch(special.DefenseID)
            {
                ...
                case SpecialDefenses.SpikeShield:
                {
                    ...
                    if(attacker is BaseCreature)
                    {
                        ...
                        defender.SendMessage("{0} odbitych obra¿eñ!", damageGiven);
                    }
                    attacker.Damage( weapon.MinDamage-=5, defender );
                    defender.SendMessage("{0} odbitych obra¿eñ!", weapon.MinDamage-=5);
                    // absorb all of the damage you would have taken
                    return damageGiven;
                }
        ...
        public XmlCustomDefenses(string name, int nspecials)
        {
            ...
            //
            if(String.Compare("Podstawa", name, true) == 0)
            {
		...
		public static bool CheckRequirements(Mobile from, SpecialDefense s)
        {
            ...
            {
                from.SendMessage("Potrzebujesz {0} si³y aby wykonaæ {1}", s.StrReq, s.Name);
                return false;
            }
            if(from.Dex < s.DexReq)
            {
                from.SendMessage("Potrzebujesz {0} zrêcznoœci aby wykonaæ {1}", s.DexReq, s.Name);
                return false;
            }
            if(from.Int < s.IntReq)
            {
                from.SendMessage("Potrzebujesz {0} inteligencji aby wykonaæ {1}", s.IntReq, s.Name);
                return false;
            }
            ...
            if(s.Skills != null && s.MinSkillLevel != null)
            {
                ...
                for(int i = 0; i < s.Skills.Length; i++)
                {
                    ...
                    if(i < s.MinSkillLevel.Length)
                    {
                        ...
                        {
                            from.SendMessage("Potrzebujsz {0} {1} aby wykonaæ {2}", s.MinSkillLevel[i], s.Skills[i].ToString(), s.Name);
                            return false;
                        }
                    } else
                    {
            ...
            {
                from.SendMessage("Potrzebujesz {0} many aby wykonaæ {1}", s.ManaReq, s.Name);
                return false;
            }
            if(from.Stam < s.StamReq)
            {
                from.SendMessage("Potrzebujesz {0} wytrzyma³oœci aby wykonaæ {1}", s.StamReq, s.Name);
                return false;
            }
            if(from.Hits < s.HitsReq)
            {
                // clear the selected defense
                from.SendMessage("Potrzebujesz {0} punktÃ³w Å¼ycia aby wykonaæ {1}", s.HitsReq, s.Name);
                return false;
            }
            ...
            if(s.Reagents != null && s.Quantity != null)
            {
                ...
                for(int i = 0; i < s.Reagents.Length; i++)
                {
                    ...
                    if(i < s.Quantity.Length)
                    {
                        ...
                        {
                            from.SendMessage("Potrzebujesz {1} {0} aby wykonaæ {2}", s.Reagents[i].Name, s.Quantity[i], s.Name);
                            return false;
                        }
                    } else
                    {
		...
		public override string OnIdentify(Mobile from)
		{
            ...
            string msg = "Umiejêtnoœci specjalne:";
            ...
            {
                msg = String.Format("{0}\nKonczy siê po {0} minutach ", msg, Expiration.TotalMinutes);
            }
		...
		private class CustomDefenseInfoGump : Gump
		{
    		...
    		public CustomDefenseInfoGump( Mobile from, XmlCustomDefenses a, SpecialDefense s) : base( 0,0)
            {
                ...
                text.AppendFormat("\n\nWymagane statystyki/umiejêtnoœci:" );
                if(s.StrReq > 0)
                {
                    text.AppendFormat("\n     {0} Si³a", s.StrReq );
                }
		...
		private class CustomDefenseGump : Gump
		{
    		...
    		public CustomDefenseGump( Mobile from, XmlCustomDefenses a) : base( 0,0)
            {
                ...
                //AddBackground( 0, 0, 70, 75, 9200 );
                //AddImageTiled(0, 0, 60, 65, 0x2624);
                //AddAlphaRegion(0, 0, 60, 65);
                AddImage( 10, 10, 0x290A );
                AddImage( 25, 10, 0x290C );
                AddLabel( 5, 52, 55, "Obrona" );
                AddLabel( 5, 67, 55, "Tarcz¹" );
                ...
                /*if(m_attachment != null && m_attachment.HasActiveCombos)
                {
                    ...
                    AddImage( 15, 90, 0x2A4E );
                } else
                {
                    // red button
                    //AddImage( 20, 10, 0x2a62 );
                    AddImage( 15, 90, 0x2A62 );
                }*/
                ...
                for(int i = 0;i<specialcount;i++)
                {
                    ...
                    */
                    ...
                    // add the info button
                    AddButton( 52, y+13, 0x5689, 0x568B, 2000+(int)s.DefenseID, GumpButtonType.Reply, 0 );
