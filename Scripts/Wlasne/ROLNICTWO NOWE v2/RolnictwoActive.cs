...
namespace Server.SkillHandlers
{
	public class RolnictwoActive
	{
		...
		{
			SkillInfo.Table[(int)SkillName.Rolnictwo].Callback = OnUse;
		}
		...
		public static TimeSpan OnUse(Mobile m)
		{
			...
			if (!m_DisableMessage)
			{
				m.SendMessage(58, "Wskaz rosline lub zwierze hodowlane?" );
			}
...
/////Pozyskiwanie nasion
if (targeted is BaseCrop)
{

BaseCrop crop = (BaseCrop)targeted;

  if (crop is CarrotCrop)
  {
     if (from.Skills.Rolnictwo.Value < 95 )
     {
     from.SendMessage( 33, "Nie udalo ci sie pozyskac nasion!" );
     }

     if ( from.Skills.Rolnictwo.Value >= 95 )
     {
     crop.Delete();
     from.AddToBackpack(new CarrotSeed( Utility.RandomMinMax( 0, 3 ) ) );
     from.SendMessage( 58, "Pozyskales troche nasion!" );
     }
  }

  if (crop is CabbageCrop)
  {
     if (from.Skills.Rolnictwo.Value < 95 )
     {
     from.SendMessage( 33, "Nie udalo ci sie pozyskac nasion!" );
     ...
     {
     crop.Delete();
     from.AddToBackpack(new CabbageSeed( Utility.RandomMinMax( 0, 3 ) ) );
     from.SendMessage( 58, "Pozyskales troche nasion!" );
     }
  }

  if (crop is LettuceCrop)
  {
     if (from.Skills.Rolnictwo.Value < 95 )
     {
     from.SendMessage( 33, "Nie udalo ci sie pozyskac nasion!" );
     ...
     {
     crop.Delete();
     from.AddToBackpack(new LettuceSeed( Utility.RandomMinMax( 0, 3 ) ) );
     from.SendMessage( 58, "Pozyskales troche nasion!" );
     }
  }

  if (crop is OnionCrop)
  {
     if (from.Skills.Rolnictwo.Value < 95 )
     {
     from.SendMessage( 33, "Nie udalo ci sie pozyskac nasion!" );
     ...
     {
     crop.Delete();
     from.AddToBackpack(new OnionSeed( Utility.RandomMinMax( 0, 3 ) ) );
     from.SendMessage( 58, "Pozyskales troche nasion!" );
     }
  }
  ...
  if (crop is CornCrop)
  {
     ...
     {
     crop.Delete();
     from.AddToBackpack(new CornSeed( Utility.RandomMinMax( 0, 3 ) ) );
     from.SendMessage( 58, "Pozyskales troche nasion!" );
     }
  }

  if (crop is WheatCrop)
  {
     if (from.Skills.Rolnictwo.Value < 95 )
     {
     from.SendMessage( 33, "Nie udalo ci sie pozyskac nasion!" );
     ...
     {
     crop.Delete();
     from.AddToBackpack(new WheatSeed( Utility.RandomMinMax( 0, 3 ) ) );
     from.SendMessage( 58, "Pozyskales troche nasion!" );
     }
  }
  ...
  if (crop is FlaxCrop)
  {
     ...
     {
     crop.Delete();
     from.AddToBackpack(new FlaxSeed( Utility.RandomMinMax( 0, 3 ) ) );
     from.SendMessage( 58, "Pozyskales troche nasion!" );
     }
  }

  if (crop is CottonCrop)
  {
     if (from.Skills.Rolnictwo.Value < 95 )
     {
     from.SendMessage( 33, "Nie udalo ci sie pozyskac nasion!" );
     ...
     {
     crop.Delete();
     from.AddToBackpack(new CottonSeed( Utility.RandomMinMax( 0, 3 ) ) );
     from.SendMessage( 58, "Pozyskales troche nasion!" );
     }
  }

  if (crop is TurnipCrop)
  {
     if (from.Skills.Rolnictwo.Value < 95 )
     {
     from.SendMessage( 33, "Nie udalo ci sie pozyskac nasion!" );
     ...
     {
     crop.Delete();
     from.AddToBackpack(new TurnipSeed( Utility.RandomMinMax( 0, 3 ) ) );
     from.SendMessage( 58, "Pozyskales troche nasion!" );
     }
  }
...
//////Oswajanie zwierzat hodowlanych
						if (creature is Sheep || creature is Bull || creature is Cow || creature is Chicken || creature is Pig || creature is Goat || creature is Rabbit )
						{
...
//////Oswajanie zwierzat hodowlanych
						...
						else if (CheckMastery(from, creature) || from.Skills[SkillName.Rolnictwo].Value >= creature.MinTameSkill)
						{
...
////
				...
				//else
				//{
				//	from.SendMessage(33, "Nie mozesz tego zrobic!");
				//}
			}
			...
			private class InternalTimer : Timer
			{
				...
				protected override void OnTick()
				{
					...
					else if (m_Count < m_MaxCount)
					{
						...
						//{
						//	m_Tamer.CheckTargetSkill(SkillName.WiedzaOBestiach, m_Creature, 0.0, 120.0);
						//}
					...
					else
					{
						...
						if (CheckMastery(m_Tamer, m_Creature) || alreadyOwned ||
							m_Tamer.CheckTargetSkill(SkillName.Rolnictwo, m_Creature, minSkill - 25.0, minSkill + 25.0))
						{
							if (m_Creature.Owners.Count == 0) // First tame
							{
								if (m_Creature is GreaterDragon)
								{
									...
									m_Creature.Skills[SkillName.Magia].Base = m_Creature.Skills[SkillName.Magia].Cap;
