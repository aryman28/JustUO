...
else if (from.Skills.Rolnictwo.Value >= 100 )
{
						...
						m_yield -= pick;
						from.SendMessage( 58, "Zebrales {0} rzepy{1}!", pick, ( pick == 1 ? "" : "" ) ); 
...
from.SendMessage( 33, "Potrzebujesz do tego Kosy!" );
		...
		private class CropTimer : Timer
		{
			...
			public CropTimer( TurnipCrop plant ) : base( TimeSpan.FromMinutes( 20 ), TimeSpan.FromMinutes( 30 ) )
			{
		...
		private class DestroyTimer : Timer
		{
			...
			public DestroyTimer( TurnipCrop plant ) : base( TimeSpan.FromHours( 20 ), TimeSpan.FromHours( 24 ) )
			{
				Priority = TimerPriority.OneSecond;
				i_plant = plant;
			}
