...
{ 
	public class BrokulySeed : BaseCrop 
	{ 
		...
		[Constructable]
		public BrokulySeed() : this( 1 )
		{
		}

		[Constructable]
		public BrokulySeed( int amount ) : base( 0xF27 )
		{
			Stackable = true; 
			Weight = .5; 
			Hue = 0x5E2; 
...
if (from.Skills.Rolnictwo.Value >= 100 )
{
			...
			this.Consume(); 
			Item item = new BrokulySeedling( from ); 
			item.Location = m_pnt; 
			item.Map = m_map; 
}			
if (from.Skills.Rolnictwo.Value < 100 )
...
from.SendMessage( 33, "Twoja wiedza jest za mala aby zasiac te rosline!");
		...
		public BrokulySeed( Serial serial ) : base( serial ) 
		{ 
		} 
	...
	public class BrokulySeedling : BaseCrop 
	{ 
		...
		[Constructable] 
		public BrokulySeedling( Mobile sower ) : base( 0xC61 ) 
		{ 
			Movable = false; 
			Name = "sadzonka brokuly"; 
			m_sower = sower;
			
			init( this );
		} 

		public static void init( BrokulySeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(BrokulyCrop), plant.Sower ); 
		...
		public BrokulySeedling( Serial serial ) : base( serial ) 
		{ 
		} 
	...
	public class BrokulyCrop : BaseCrop 
	{ 
		...
		[Constructable] 
		public BrokulyCrop( Mobile sower ) : base(  0xCC7 ) 
		{ 
			Movable = false; 
			Name = "brokuly"; 
                        Hue = 0x48F;
		...
		public static void init ( BrokulyCrop plant, bool full )
		{
...
pick = m_yield;
						...
						m_yield -= pick;
						from.SendMessage( 58, "Zebrales {0} Brokuly{1}!", pick, ( pick == 1 ? "" : "" ) ); 

						((Item)this).ItemID = pickedGraphic;

                                                if ( pick > 0 )
						{
						Brokuly crop = new Brokuly( pick ); 
...
from.SendMessage( 33, "Potrzebujesz do tego Kosy!" );
		...
		{
			private BrokulyCrop i_plant;

			public CropTimer( BrokulyCrop plant ) : base( TimeSpan.FromMinutes( 20 ), TimeSpan.FromMinutes( 30 ) )
			{
		...
		private class DestroyTimer : Timer
		{
			...
			public DestroyTimer( BrokulyCrop plant ) : base( TimeSpan.FromHours( 20 ), TimeSpan.FromHours( 24 ) )
			{
				Priority = TimerPriority.OneSecond;
				i_plant = plant;
			}
		...
		public BrokulyCrop( Serial serial ) : base( serial ) 
		{ 
		} 
