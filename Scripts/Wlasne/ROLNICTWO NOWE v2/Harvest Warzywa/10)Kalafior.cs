...
{ 
	public class KalafiorSeed : BaseCrop 
	{ 
		...
		[Constructable]
		public KalafiorSeed() : this( 1 )
		{
		}

		[Constructable]
		public KalafiorSeed( int amount ) : base( 0xF27 )
		{
			Stackable = true; 
			Weight = .5; 
			Hue = 0x5E2; 
...
if (from.Skills.Rolnictwo.Value >= 100 )
{
			...
			this.Consume(); 
			Item item = new KalafiorSeedling( from ); 
			item.Location = m_pnt; 
			item.Map = m_map; 
}			
if (from.Skills.Rolnictwo.Value < 100 )
...
from.SendMessage( 33, "Twoja wiedza jest za mala aby zasiac te rosline!");
		...
		public KalafiorSeed( Serial serial ) : base( serial ) 
		{ 
		} 
	...
	public class KalafiorSeedling : BaseCrop 
	{ 
		...
		[Constructable] 
		public KalafiorSeedling( Mobile sower ) : base( 0xCB5 ) 
		{ 
			Movable = false; 
			Name = "sadzonka kalafiora"; 
			m_sower = sower;
			
			init( this );
		} 

		public static void init( KalafiorSeedling plant )
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(KalafiorCrop), plant.Sower ); 
		...
		public KalafiorSeedling( Serial serial ) : base( serial ) 
		{ 
		} 
	...
	public class KalafiorCrop : BaseCrop 
	{ 
		...
		[Constructable] 
		public KalafiorCrop( Mobile sower ) : base( 0xD06 )
		{ 
			Movable = false; 
			Name = "kalafior"; 
                        Hue = 0x000;
		...
		public static void init ( KalafiorCrop plant, bool full )
		{
...
else if (from.Skills.Rolnictwo.Value >= 100 )
{
						...
						m_yield -= pick;
						from.SendMessage( 58, "Zebrales {0} kalafior{1}!", pick, ( pick == 1 ? "" : "" ) ); 

						((Item)this).ItemID = pickedGraphic;

						Kalafior crop = new Kalafior( pick ); 
...
from.SendMessage( 33, "Potrzebujesz do tego Kosy!" );
		...
		{
			private KalafiorCrop i_plant;

			public CropTimer( KalafiorCrop plant ) : base( TimeSpan.FromMinutes( 20 ), TimeSpan.FromMinutes( 30 ) )
			{
		...
		private class DestroyTimer : Timer
		{
			...
			public DestroyTimer( KalafiorCrop plant ) : base( TimeSpan.FromHours( 20 ), TimeSpan.FromHours( 24 ) )
			{
				Priority = TimerPriority.OneSecond;
				i_plant = plant;
			}
		...
		public KalafiorCrop( Serial serial ) : base( serial ) 
		{ 
		} 
