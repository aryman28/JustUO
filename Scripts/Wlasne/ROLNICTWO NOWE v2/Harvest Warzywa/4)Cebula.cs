...
if (from.Skills.Rolnictwo.Value >= 60 )
{
...
BaseAxe kosa = from.FindItemOnLayer(Layer.OneHanded) as BaseAxe;
  ...
  from.SendMessage( 33, "Kosa rozpadla sie!" ); 
			...
			if ( DateTime.Now > lastpicked.AddSeconds(3) ) // 3 seconds between picking
			{
              ...
              from.CheckSkill( SkillName.Rolnictwo, 50.0, 70.0 );
...
else if (from.Skills.Rolnictwo.Value >= 50 )
{

						 //int pick = Utility.Random( m_yield + 1 );
			                         int pick = 1;

if (from.Skills.Rolnictwo.Value >= 70 && m_yield >= 4 || from.Skills.Rolnictwo.Value >= 70 && m_yield >= 3 || from.Skills.Rolnictwo.Value >= 70 && m_yield >= 2 )
{
pick = 2;
}
...
pick = m_yield;
						...
						m_yield -= pick;
						from.SendMessage( 58, "Zebrales {0} cebuli{1}!", pick, ( pick == 1 ? "" : "" ) ); 
...
from.SendMessage( 33, "Potrzebujesz do tego Kosy!" );
		...
		private class CropTimer : Timer
		{
			...
			public CropTimer( OnionCrop plant ) : base( TimeSpan.FromMinutes( 20 ), TimeSpan.FromMinutes( 30 ) )
			{
		...
		private class DestroyTimer : Timer
		{
			...
			public DestroyTimer( OnionCrop plant ) : base( TimeSpan.FromHours( 20 ), TimeSpan.FromHours( 24 ) )
			{
				Priority = TimerPriority.OneSecond;
				i_plant = plant;
			}
