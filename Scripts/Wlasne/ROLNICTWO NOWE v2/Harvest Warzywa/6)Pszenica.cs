...
if (from.Skills.Rolnictwo.Value >= 80 )
{
...
BaseAxe kosa = from.FindItemOnLayer(Layer.OneHanded) as BaseAxe;
  ...
  from.SendMessage( 33, "Kosa rozpadla sie!" ); 
			...
			if ( DateTime.Now > lastpicked.AddSeconds(3) ) // 3 seconds between picking
			{
              ...
              from.CheckSkill( SkillName.Rolnictwo, 70.0, 90.0 );
...
if (from.Skills.Rolnictwo.Value < 70 )
{
from.SendMessage( 33, "Nie udalo ci sie tego zebrac!");
}
else if (from.Skills.Rolnictwo.Value >= 70 )
{
						int pick = 1;
						...
						m_yield -= pick;
						from.SendMessage( 58, "Zebrales {0} pszenicy{1}!", pick, ( pick == 1 ? "" : "" ) ); 
...
from.SendMessage( 33, "Potrzebujesz do tego Kosy!" );
		...
		private class CropTimer : Timer
		{
			...
			public CropTimer( WheatCrop plant ) : base( TimeSpan.FromMinutes( 20 ), TimeSpan.FromMinutes( 30 ) )
			{
		...
		private class DestroyTimer : Timer
		{
			...
			public DestroyTimer( WheatCrop plant ) : base( TimeSpan.FromHours( 20 ), TimeSpan.FromHours( 24 ) )
			{
				Priority = TimerPriority.OneSecond;
				i_plant = plant;
			}
