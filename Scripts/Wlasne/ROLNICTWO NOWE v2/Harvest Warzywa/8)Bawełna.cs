...
BaseAxe kosa = from.FindItemOnLayer(Layer.OneHanded) as BaseAxe;
  ...
  from.SendMessage( 33, "Kosa rozpadla sie!" ); 
			...
			if ( DateTime.Now > lastpicked.AddSeconds(3) ) // 3 seconds between picking
			{
              ...
              from.CheckSkill( SkillName.Rolnictwo, 90.0, 100.0 );
...
else if (from.Skills.Rolnictwo.Value >= 90 )
{
...
pick = m_yield;
						...
						m_yield -= pick;
						from.SendMessage( 58, "Zebrales {0} bawelny{1}!", pick, ( pick == 1 ? "" : "" ) ); 
...
from.SendMessage( 33, "Potrzebujesz do tego Kosy!" );
		...
		private class CropTimer : Timer
		{
			...
			public CropTimer( CottonCrop plant ) : base( TimeSpan.FromMinutes( 20 ), TimeSpan.FromMinutes( 30 ) )
			{
		...
		private class DestroyTimer : Timer
		{
			...
			public DestroyTimer( CottonCrop plant ) : base( TimeSpan.FromHours( 20 ), TimeSpan.FromHours( 24 ) )
			{
				Priority = TimerPriority.OneSecond;
				i_plant = plant;
			}
