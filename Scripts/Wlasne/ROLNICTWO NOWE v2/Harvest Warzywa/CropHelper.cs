...
{ 
	public class BaseCrop : Item
	{
		...
		public virtual TimeSpan SowerPickTime{ get{ return TimeSpan.FromMinutes( 1 ); } }  //Changed from 14 to 2
	...
	{ 
		...
		public static bool ValidateGardenPlot( Map map, int x, int y )
		{
			...
			foreach( Item item in eable )
			{
				//if( item.ItemID == 0x32C9 ) // dirt; possibly also 0x32CA 
                                  if( item.Hue == 2110 )      // roœnie tylko na podlanym gruncie ogrodu
					ground = true;
			}
			eable.Free();
		...
		{ 
			...
			private int cnt;
			private double rolnictwoValue;
			private double rnd;

			public GrowTimer( Item seedling, Type croptype, Mobile sower ) : base( TimeSpan.FromSeconds( 60 ), TimeSpan.FromSeconds( 12 ) ) 
			{ 
				...
				rolnictwoValue = sower.Skills[SkillName.Rolnictwo].Value / 60;
			} 
			...
			{ 
				if ( cnt++ / 20 > rnd ) // between 10 and 30 minutes changed from 100 to 500
				{
					...
					{ 
						...
						if ( newitem == null || Utility.RandomDouble() > rolnictwoValue )
						{
		...
		{ 
			...
			{ 
				...
				{ 
					if (( from == m_sower ) || ( DateTime.Now >= planted.AddSeconds(120) )) //changed from Days
					{
