...
BaseAxe kosa = from.FindItemOnLayer(Layer.OneHanded) as BaseAxe;
  ...
  from.SendMessage( 33, "Kosa rozpadla sie!" ); 
			...
			if ( DateTime.Now > lastpicked.AddSeconds(3) ) // 3 seconds between picking
			{
              ...
              from.CheckSkill( SkillName.Rolnictwo, 60.0, 80.0 );
...
else if (from.Skills.Rolnictwo.Value >= 60 )
{
...
pick = m_yield;
						...
						m_yield -= pick;
						from.SendMessage( 58, "Zebrales {0} kukurydze{1}!", pick, ( pick == 1 ? "" : "" ) ); 
...
from.SendMessage( 33, "Potrzebujesz do tego Kosy!" );
		...
		private class CropTimer : Timer
		{
			...
			public CropTimer( CornCrop plant ) : base( TimeSpan.FromMinutes( 20 ), TimeSpan.FromMinutes( 30 ) )
			{
		...
		private class DestroyTimer : Timer
		{
			...
			public DestroyTimer( CornCrop plant ) : base( TimeSpan.FromHours( 20 ), TimeSpan.FromHours( 24 ) )
			{
				Priority = TimerPriority.OneSecond;
				i_plant = plant;
			}
