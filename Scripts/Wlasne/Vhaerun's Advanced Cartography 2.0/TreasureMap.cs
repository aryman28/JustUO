...
namespace Server.Items
{
	public class TreasureMap : MapItem
	{
		...
		public const double LootChance = 0.09; // 9% chance to appear as loot
		...
		public static bool HasDiggingTool( Mobile m )
		{
			...
			{
				if ( tool.HarvestSystem == Engines.Harvest.Gornictwo.System )
					return true;
			}
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				*/
				...
				else
				{
					...
					int maxRange;
					double skillValue = from.Skills[SkillName.Gornictwo].Value;
					...
					if ( Utility.InRange( targ3D, chest3D0, maxRange ) )
					{
						...
						{
							from.SendMessage( "Nie mozesz wykopac skrzyni poniewaz stoisz na niej." ); // The chest can't be dug up because you are standing on top of it.
						}
						else if ( map != null )
						{
							int z = map.GetAverageZ( x, y );
					...
					else
					{
						if ( Utility.InRange( targ3D, chest3D0, 8 ) ) // We're close, but not quite
						{
							from.SendAsciiMessage( 0x44, "Skrzynia ze skarbem jest bardzo blisko!" );
						}
						else
						{
		...
		private class DigTimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				{
					m_From.SendMessage( "Przerwales kopanie poniewaz cos jest dokladnie nad skrzynia." ); // You stop digging because something is directly on top of the treasure chest.
					Terminate();
					return;
				}
				...
				if ( m_Count > 1 && m_Dirt1 == null )
				{
					...
					m_Dirt1.MoveToWorld( m_Location, m_Map );
                                        //m_Dirt1.MoveToWorld( new Point3D( m_Location.X - 1, m_Location.Y, m_Location.Z ), m_Map );

					m_Dirt2 = new TreasureChestDirt();
                                        m_Dirt2.MoveToWorld( m_Location, m_Map );
				...
				//if ( m_Chest != null && m_Chest.Location.Z >= m_Location.Z + 5 )
                                if ( m_Chest != null && m_Chest.Location.Z >= 0 )
				{
					Stop();
					m_From.EndAction( typeof( TreasureMap ) );
		...
		{
			return ( from.Skills[SkillName.Kartografia].Value >= GetMinSkillLevel() );
		}
		...
		public void Decode( Mobile from )
		{
			...
			else
			{
				...
				if ( from.Skills[SkillName.Kartografia].Value < minSkill )
					from.SendLocalizedMessage( 503013 ); // The map is too difficult to attempt to decode.

				double maxSkill = minSkill + 60.0;

				if ( !from.CheckSkill( SkillName.Kartografia, minSkill, maxSkill ) )
				{
		...
		private class DigEntry : ContextMenuEntry
		{
			...
			public override void OnClick()
			{
				...
				if ( HasDiggingTool( from ) )
					...
					from.SendMessage( "Musisz miec narzedzie do kopania aby wykopac skarb." );
			}
		}
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			if ( m_Completed )
			{
				list.Add( 1041507, m_CompletedBy == null ? "ktos" : m_CompletedBy.Name ); // completed by ~1_val~
			}
		}
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( m_Completed )
			{
				from.Send( new MessageLocalizedAffix( Serial, ItemID, MessageType.Label, 0x3B2, 3, 1048030, "", AffixType.Append, String.Format( " skompletowana przez {0}", m_CompletedBy == null ? "ktos" : m_CompletedBy.Name ), "" ) );
			}
			...
			{
				if ( m_Level == 6 )
					LabelTo( from, 1041522, String.Format( "#{0}\t \t#{1}", 1063452, m_Map == Map.Ilshenar, "Valeria" ) );
				else
					LabelTo( from, 1041522, String.Format( "#{0}\t \t#{1}", 1041510 + m_Level, m_Map == Map.Ilshenar, "Valeria" ) );
			}
		}
		...
		private class CartGump : Gump
		{
			...
			public CartGump( TreasureMap tmap, MessageEntry entry, Map map, Point2D loc ) : base( (640 - entry.Width) / 2, (480 - entry.Height) / 2 )
			{
				...
				AddHtml( 38, 38, entry.Width - 83, entry.Height - 86, String.Format( "Udalo ci sie odczytac koordynaty miejsca zakopania skarbu:" ), false, false );
