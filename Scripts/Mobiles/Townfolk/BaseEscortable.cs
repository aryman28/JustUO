...
namespace Server.Mobiles
{
	public class BaseEscortable : BaseCreature
	{
		...
		private static string[] m_TownNames = new string[]
			{
				"Azzan", "Valeria", "Barna",
				"Gusulia", "Okaria", "Linhal", "ZakonPaladynow"
		...
		public virtual bool SayDestinationTo( Mobile m )
		{
			...
			if ( escorter == null )
			{
				Say( "Musze dostac sie do {0}, zaprowadzisz mnie?", (dest.Name == "ZakonPaladynow" && m.Map == Map.Ilshenar) ? "Zakon Paladynow" : dest.Name );
				return true;
			}
			else if ( escorter == m )
			{
				Say( "Ruszajmy! Zaplace jak dojdziemy do {0}.", (dest.Name == "ZakonPaladynow" && m.Map == Map.Ilshenar) ? "Zakon Paladynow" : dest.Name );
				return true;
			}
		...
		public virtual bool AcceptEscorter( Mobile m )
		{
			...
			{
				Say( "Widze ze masz juz kogos ze soba." );
				return false;
			}
			else if ( m is PlayerMobile && (((PlayerMobile)m).LastEscortTime + m_EscortDelay) >= DateTime.Now )
			{
				...
				Say( "Musisz odpoczac {0} minut{1} przed kolejna wyprawa.", minutes, minutes == 1 ? "" : "" );
				return false;
			}
		...
		{
			if ( Map.Felucca.Regions.Count == 0 || Map.Ilshenar.Regions.Count == 0 || Map == null || Map == Map.Internal || Location == Point3D.Zero )
		...
		public EDI GetDestination()
		{
			...
			if ( Map.Ilshenar.Regions.Count > 0 || Map.Felucca.Regions.Count > 0 )
	...
	public class EscortDestinationInfo
	{
		...
		{
			ICollection list = Map.Ilshenar.Regions.Values;
                        ICollection list1 = Map.Felucca.Regions.Values;
