...
namespace Server.Mobiles
{
	public class NecroAI : BaseAI
	{
		...
		public virtual double ScaleByNekromancja( double v )
		{
			return m_Mobile.Skills[SkillName.Nekromancja].Value * v * 0.01;
		}
		...
		public override bool DoActionWander()
		{
			...
			else if ( SmartAI && m_Mobile.Mana < m_Mobile.ManaMax )
			{
				...
				m_Mobile.UseSkill( SkillName.Medytacja );
			}
			else
			{
				...
				else if ( !m_Mobile.Summoned && (SmartAI || (ScaleByNekromancja( HealChance ) > Utility.RandomDouble())) )
				{
					if ( m_Mobile.Hits < (m_Mobile.HitsMax - 50) )
					{
						m_Mobile.UseSkill( SkillName.MowaDuchow );
					}
		...
		{
			if ( !m_Mobile.DisallowAllMoves && (SmartAI ? Utility.Random( 4 ) == 0 : ScaleByNekromancja( TeleportChance ) > Utility.RandomDouble()) )
			{
		...
		{
			int maxCircle = (int)((m_Mobile.Skills[SkillName.Nekromancja].Value + 50.0) / (100.0 / 7.0));
		...
		public virtual Spell DoDispel( Mobile toDispel )
		{
			if ( !SmartAI )
			{
				if ( ScaleByNekromancja( DispelChance ) > Utility.RandomDouble() )
		...
		public virtual Spell ChooseSpell( Mobile c )
		{
			if ( !SmartAI )
			{
				if ( !m_Mobile.Summoned && ScaleByNekromancja( HealChance ) > Utility.RandomDouble() )
				{
					if ( m_Mobile.Hits < (m_Mobile.HitsMax - 50) )
						m_Mobile.UseSkill( SkillName.MowaDuchow );
					else if ( m_Mobile.Hits < (m_Mobile.HitsMax - 10) )
		...
		public override bool DoActionCombat()
		{
			...
			if ( SmartAI && !m_Mobile.StunReady && m_Mobile.Skills[SkillName.Boks].Value >= 80.0 && m_Mobile.Skills[SkillName.Anatomia].Value >= 80.0 )
			...
			if ( m_Mobile.Spell == null && DateTime.Now > m_NextCastTime && m_Mobile.InRange( c, 12 ) )
			{
				...
				if ( spell != null && spell.Cast() )
				{
					...
					{
						double del = ScaleByNekromancja( 3.0 );
