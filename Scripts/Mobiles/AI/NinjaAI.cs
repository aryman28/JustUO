...
namespace Server.Mobiles
{
	public class NinjaAI : BaseAI
	{
		...
		{
			return (m_Mobile.Skills[ SkillName.Skrytobojstwo ].Value/1000.0);
		}

		private bool CanUseAbility( double limit, int mana, double chance )
		{
			if ( m_Mobile.Skills[ SkillName.Skrytobojstwo ].Value >= limit && m_Mobile.Mana >= mana )
			{
		...
		{
			if ( m_Mobile.Skills[ SkillName.Skrytobojstwo ].Value < 50.0 )
			{
				return false;
			}
			...
			if ( toTarget != null )
			{
				...
				{
					double outerradius = m_Mobile.Skills[ SkillName.Skrytobojstwo ].Value/10.0;
		...
		public override bool DoActionWander()
		{
			...
			if ( turnOrHideChance > Utility.RandomDouble() && !m_Mobile.Hidden )
			{
				...
				{
					PerformHide();
					m_Mobile.UseSkill( SkillName.Zakradanie );
				}
			}
		...
		public override bool DoActionCombat()
		{
			...
			if ( dstToTarget > 2.0 && dstToTarget <= (m_Mobile.Skills[ SkillName.Skrytobojstwo ].Value/10.0) && m_Mobile.Mana > 45 && comboChance > (Utility.RandomDouble() + MagnitudeBySkill()) )
			{
				PerformHide();
