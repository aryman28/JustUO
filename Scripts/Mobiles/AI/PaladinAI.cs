...
namespace Server.Mobiles
{
	public class PaladinAI : BaseAI
	{
		...
		public virtual double ScaleByRycerstwo( double v )
		{
			return m_Mobile.Skills[SkillName.Rycerstwo].Value * v * 0.01;
		}

		public virtual double ScaleByMagia( double v )
		{
			return m_Mobile.Skills[SkillName.Magia].Value * v * 0.01;
		}
		...
		public virtual double ScaleByHealing( double v )
		{
			...
			scaled = ( m_Mobile.Skills[SkillName.Magia].Value + m_Mobile.Skills[SkillName.Rycerstwo].Value )* v * 0.005;
		...
		{
			if ( !m_Mobile.DisallowAllMoves && (SmartAI ? Utility.Random( 4 ) == 0 : ScaleByMagia( TeleportChance ) > Utility.RandomDouble()) )
			{
		...
		{
			int maxCircle = (int)((m_Mobile.Skills[SkillName.Rycerstwo].Value + 20.0) / (100.0 / 7.0));
			int minCircle = (int)((m_Mobile.Skills[SkillName.Magia].Value + 20.0) / (100.0 / 7.0));
		...
		public override bool DoActionCombat()
		{
			...
			if ( SmartAI && !m_Mobile.StunReady && m_Mobile.Skills[SkillName.WalkaMieczami].Value >= 80.0 && m_Mobile.Skills[SkillName.Anatomia].Value >= 80.0 )
			...
			if ( m_Mobile.Spell == null && DateTime.Now > m_NextCastTime && m_Mobile.InRange( c, 12 ) )
			{
				...
				if ( spell != null && spell.Cast() )
				{
					...
					{
						double del = ScaleByRycerstwo( 3.0 );
