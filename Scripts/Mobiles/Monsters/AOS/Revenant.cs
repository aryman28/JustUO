...
namespace Server.Mobiles
{
	public class Revenant : BaseCreature
	{
			...
			: base(AIType.AI_Melee, FightMode.Closest, 10, 1, 0.18, 0.36)
		{
			...
			double scalar = caster.Skills[SkillName.MowaDuchow].Value * 0.01;
			...
			SetSkill(SkillName.ObronaPrzedMagia, 100.0 * scalar); // magic resist is absolute value of spiritspeak
			SetSkill(SkillName.Taktyka, 100.0); // always 100
			SetSkill(SkillName.WalkaMieczami, 100.0 * scalar); // not displayed in animal lore but tests clearly show this is influenced
			SetSkill(SkillName.Wykrywanie, 75.0 * scalar);
		...
		public override void OnThink()
		{
			...
			if (m_Target.Hidden && InRange(m_Target, 3) && Core.TickCount >= NextSkillTime && UseSkill(SkillName.Wykrywanie))
			{
				Target targ = Target;
